...
inline uint8_t rc_to_idx(int row, int col) {
  return static_cast<uint8_t>((row & 3) * BOARD_W + (col & 3));
}

// Helper functions to convert a bitboard index back to a row or column.
inline int idx_r(uint8_t index) { return index / BOARD_W; }
inline int idx_c(uint8_t index) { return index % BOARD_W; }

// Creates a bitmask with a single bit set at the specified index.
inline bb_t bit(uint8_t index) { return static_cast<bb_t>(1u) << index; }
