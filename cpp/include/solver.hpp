#pragma once
#include <cstdint>
#include <unordered_map>
#include "bitboard.hpp"
#include "hash.hpp"

namespace collapsi {

struct Answer { bool win; uint8_t best_move; }; // 0xFF none

class Solver {
public:
  Answer solve(const BitState& s);

private:
  std::unordered_map<Key64, Answer, Key64Hasher> cache_;
  Answer solve_rec(const BitState& s);
};

// Heuristic: fewest opponent replies, prefer count==1 first
inline uint8_t encode_move(uint8_t from, uint8_t to) { return static_cast<uint8_t>(((from & 0xF) << 4) | (to & 0xF)); }
inline uint8_t move_from(uint8_t m) { return (m >> 4) & 0xF; }
inline uint8_t move_to(uint8_t m) { return m & 0xF; }

}


