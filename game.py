...
def deal_board_4x4(seed: Optional[int] = None) -> Tuple[Board, Coord, Coord]:
    ...
    # Deck: J, J, A, A, A, A, 2x4, 3x4, 4x2 = 16
    deck: List[Card] = ['J', 'J'] + ['A'] * 4 + ['2'] * 4 + ['3'] * 4 + ['4'] * 2
    rng.shuffle(deck)
    width = height = 4
    board = Board(width=width, height=height, grid=tuple(deck))
    # First two J positions are start cells for P1, P2 in reveal order
...
def deal_board_3x3(seed: Optional[int] = None) -> Tuple[Board, Coord, Coord]:
    rng = random.Random(seed)
    # User requested: 2 Jacks, 4 Aces, 4 Twos for 3x3 (9 cells) which totals 10.
    # Assumption: drop one '2' to fit 9 cells → deck: 2J, 4A, 3x'2'.
...
class GameState:
    board: Board
    collapsed: Tuple[Coord, ...]  # immutable set as tuple sorted
    p1: Coord
    p2: Coord
    turn: int  # 1 or 2
...
def enumerate_destinations(state: GameState, start: Coord, steps: int, opponent: Coord) -> Set[Coord]:
    # DFS on paths of exactly 'steps' moves with constraints
...
# Provide an example orthogonal wrap-around path for a given destination
...
class SolveResult:
    ...
    proof_moves: Optional[Dict[GameState, Coord]]  # minimal strategy (sparse)
...
def choose_child_by_heuristic(state: GameState, moves: List[Coord]) -> List[Coord]:
    # User-specified heuristic: prefer first child where opponent has exactly 1 move;
    # otherwise choose ascending by opponent move count.
    scored = [(m, opponent_move_count_after(state, m)) for m in moves]
    for m, cnt in scored:
        if cnt == 1:
            # Put this first, keep original relative order for others by count
...
def aostar_solve(state: GameState, transposition: Optional[Dict[GameState, bool]] = None, depth_cap: Optional[int] = None) -> SolveResult:
        ...
        for move in ordered:
            ...
            for o in choose_child_by_heuristic(s_after, opp_moves):
                ...
                if not solve_rec(s_after_opp, depth + 2):
                    # Opponent has a refutation (we cannot force a win after this reply)
    ...
    if can_win:
        # pick the first winning move according to heuristic
...
def choose_ai_side_for_board(board: Board, p1: Coord, p2: Coord, db_path: str) -> int:
    ...
    return 2 if res_p2.win else 1  # default to 1 if neither stored as winning
